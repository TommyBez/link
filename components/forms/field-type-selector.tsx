...
const fieldTypes: Array<{
  ...
  { type: "heading", label: "Heading", icon: Heading, defaultLabel: "Section Title" },
  { type: "text_block", label: "Text Block", icon: FileText, defaultLabel: "Information text..." },
  { type: "divider", label: "Divider", icon: Minus, defaultLabel: "" },
  { type: "text", label: "Text Input", icon: Type, defaultLabel: "Text field" },
  { type: "textarea", label: "Text Area", icon: AlignLeft, defaultLabel: "Long text field" },
  ...
  { type: "checkbox", label: "Checkbox", icon: CheckSquare, defaultLabel: "Checkbox option" },
  { type: "radio", label: "Radio", icon: Circle, defaultLabel: "Radio options" },
  { type: "select", label: "Select", icon: ChevronDown, defaultLabel: "Select option" },
...
export function FieldTypeSelector({ onAddField }: FieldTypeSelectorProps) {
  const handleAddField = (type: FieldType, defaultLabel: string) => {
    ...
    onAddField({
      ...
      required: false,
      options: ["radio", "select"].includes(type) ? ["Option 1", "Option 2"] : undefined,
    })
  }

  return (
    <div className="rounded-lg border border-gray-200 bg-white p-6">
      <h2 className="mb-4 text-lg font-semibold text-gray-900">Add Field</h2>
        ...
        {fieldTypes.map((fieldType) => {
          ...
          return (
            <Button
              key={fieldType.type}
              variant="outline"
              onClick={() => handleAddField(fieldType.type, fieldType.defaultLabel)}
