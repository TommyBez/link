    ...
    <Document>
      <Page size="A4" style={styles.page}>
        <View style={styles.header}>
          ...
          {data.formDescription && <Text style={styles.description}>{data.formDescription}</Text>}
          <Text style={styles.submissionInfo}>
            Submitted by: {data.clientName} ({data.clientEmail})
          </Text>
          <Text style={styles.submissionInfo}>Date: {new Date(data.submittedAt).toLocaleString()}</Text>
        </View>
          ...
          {data.fields.map((field, index) => {
            ...
            if (field.fieldType === "signature" && field.answer) {
              return (
                <View key={index} style={styles.field}>
                  ...
                  <Image src={field.answer || "/placeholder.svg"} style={styles.signature} />
                </View>
              )
            }
            ...
            return (
              <View key={index} style={styles.field}>
                ...
                <Text style={styles.value}>{field.answer || "(No answer provided)"}</Text>
              </View>
            )
          })}
        </View>
